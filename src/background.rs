...
pub fn initialize_background(
    ...
    let sprite = |transform| {
        ...
        commands.spawn((
            sprite(Transform::from_xyz(constants::MAX_X + constants::HALF_WIDTH, 0., 0.)),
            Background,
        ));
...
pub fn background_setup(
    mut commands: Commands, 
    images: Res<BackgroundImg>,
) {
    commands.spawn(Camera2dBundle {
        projection: OrthographicProjection {
            scaling_mode: ScalingMode::Fixed { width: constants::WIDTH, height: constants::HEIGHT },
            ..default()
        }.into() ,
        camera: Camera {
...
pub fn move_background(
    ...
    let (sprite1, sprite2) = query_sprites.iter_mut().collect_tuple().expect("two sprites should be here");
    let (mut transform_left, mut transform_right) = 
        if sprite1.translation.x < sprite2.translation.x {
            (sprite1, sprite2)
        } else {
            (sprite2, sprite1) 
        };
